...
Prometheus-operator не поддерживает прямую отправку алертов в Telegram, поэтому Alertmanager настраивается на отправку алертов через webhook и деплоится приложение, которое отправляет полученные данные в Telegram.
...
spec:
 ...
 ports:
   - protocol: TCP
     port: 8080
```
`TELEGRAM_CHAT_ID` и `TELEGRAM_TOKEN` необходимо поставить свои. Подробнее о Telegram API можно почитать тут - https://core.telegram.org/bots.
